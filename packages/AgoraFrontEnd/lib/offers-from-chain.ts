...
/**
 * Cross-market variant of `parseOfferReadResults` — keeps every active offer
 * regardless of `marketId`. Used by the Portfolio page to compute best bids
 * per market and the user's escrow positions across all markets in one read.
 */
export function parseAllActiveOffers(
  startOfferId: bigint,
